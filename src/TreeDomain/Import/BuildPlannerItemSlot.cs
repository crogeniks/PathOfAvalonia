...
/// <summary>Canonical mapping between PoB equipment slot labels and Build Planner inventory ids.</summary>
...
public static class BuildPlannerItemSlots
{
    ...
    [
        ...
        new("Flask 1", "Flask1", 12), new("Flask 2", "Flask2", 13), new("Flask 3", "Flask3", 14),
        new("Flask 4", "Flask4", 15), new("Flask 5", "Flask5", 16),
        new("Charm 1", "Charm1", 17), new("Charm 2", "Charm2", 18),
    ];
    ...
    public static int SortOrder(string? displayName) =>
        TryGetByDisplayName(displayName, out var slot) ? slot.SortOrder : int.MaxValue;
}
